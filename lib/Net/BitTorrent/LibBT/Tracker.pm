package Net::BitTorrent::LibBT::Tracker;
...
C<libbttracker> Hash and Peer databases and the statistics stored in shared
memory.
...
=head3 C<< $tracker = Net::BitTorrent::LibBT::Tracker->new($dir, [$master]) >>

Connects to, or creates, a LibBTTracker tracker database.
...
=head3 C<< Net::BitTorrent::LibBT::Tracker::Flags() >>
...
Returns a C<Net::BitTorrent::LibBT::Tracker::Config> object which allows
the tracker's configuration data to be retrieved and set. Consult the
C<libbttracker> documentation for the meanings of each of these configuration
values.

=head4 C<< Net::BitTorrent::LibBT::Tracker::Config >> Methods
...
defined by the L</Net::BitTorrent::LibBT::Tracker::Flags()> function; some are used internally. You can 
...
If the LibBTTtracker is embedded inside another server, that server will set this
...
Returns a C<Net::BitTorrent::LibBT::Tracker::Stats> object which may be used
to fetch (and set) statistics on the tracker's internal operation.

=head4 C<Net::BitTorrent::LibBT::Tracker::Stats> Methods
...
Returns a Net::BitTorrent::LibBT::Tracker::Infohash object for the given 20-byte $hash_id.
If $create is set to "1", the hash will be created if it does not already exist.

=item C<< @hashes = $tracker->Infohashes() >>

Returns an array of C<Net::BitTorrent::LibBT::Tracker::Infohash> objects, one for each hash
...
Copy changes to the C<Net::BitTorrent::LibBT::Tracker::Infohash> object back into the tracker
database.
...
=item C<< Net::BitTorrent::LibBT::Tracker::Peer::Flags() >>
...
Given the 20-byte peer_id, return a C<Net::BitTorrent::LibBT::Tracker::Peer> object from the database
...
to access C<Net::BitTorrent::LibBT::Tracker::Peer> objects after the C<Net::BitTorrent::LibBT::Tracker::Infohash> object
...
L</Net::BitTorrent::LibBT::Tracker::Peer::Flags()> function.
...
While C<libbttracker> itself is, C<Net::BitTorrent::LibBT::Tracker> is
B<not> transactional.
